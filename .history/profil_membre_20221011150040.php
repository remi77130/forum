...
<div class="modal-content">
    ...
    <p>Je recherche des </p>
        <label>Homme
        <input type="checkbox" name="H" id="checkMen"><br>
        </label> <br>

        <label>Femme
        <input type="checkbox" name="F" id="checkWomen"> <br>
        </label>
...
<script src="https://code.jquery.com/jquery-3.6.1.slim.min.js" integrity="sha256-w8CvhFs7iHNVUtnSP0YKEg00p9Ih13rlL9zGqvLdePA=" crossorigin="anonymous"></script>                        
    <script>
        ...
        // Filtre de recherche Homme
        
        var checkboxHomme = document.getElementById("checkMen")

        checkboxHomme.addEventListener('change',()=>{
            // Si checkbox filtre homme = coché
            if(event.currentTarget.checked == true){
                var elems = $('.container_profil_membre[data-sexe="Homme"]')
                $('.container_profil_membre').not(elems).hide()
                elems.show()
            }else{ // Si checkbox filtre homme = pas coché
                // Si check femme coché alors on affiche que les femmes
                if($('#checkWomen').is(':checked')){
                    var elems = $('.container_profil_membre[data-sexe="Femme"]')
                    $('.container_profil_membre').not(elems).hide()
                    elems.show()
                }else{  // Si checkbox femme pas coché alors on affiche tous les profils
                    var elems = $('.container_profil_membre')
                    elems.show()
                }
        ...
        // Filtre de recherche Femme
        
        var checkboxFemme = document.getElementById("checkWomen")

        checkboxFemme.addEventListener('change',()=>{
            // Si checkbox filtre femme = coché
            if(event.currentTarget.checked == true){
                var elems = $('.container_profil_membre[data-sexe="Femme"]')
                $('.container_profil_membre').not(elems).hide()
                elems.show()
            }else{ // Si checkbox filtre femme = pas coché
                // Si checkbox homme coché alors on affiche que les hommes

                // Sinon on affiche tous les profils
            }
        })

        // Filtre de recherche âge
        var inputMax = document.getElementById("age_max");

        inputMax.addEventListener('input', function() {
            $minVal = parseInt(document.getElementById("age_min").value);
            $maxVal = parseInt(document.getElementById("age_max").value);
            console.log($minVal)
            $('container_profil_membre[data-age]').filter(function(){
                console.log($(this).data('age') <= $minVal)
                return $(this).data('age') <= $minVal
            }).show()
        })
       

        // Filtre de recherche departement
