...
if(!empty($_POST['search'])) {
    ...
    if(!empty($_POST['departement'])) {
        ...
        $departement_code = intval(htmlspecialchars($_POST['departement'])) ?? -1;
        if($departement_code!=-1){
            $reqdpt = $bdd->prepare("SELECT *
            FROM departement
            WHERE departement_code = ?");
            $reqdpt->execute(array($departement_code));
            $departement = $reqdpt->fetch();
            $dpcode = $departement['departement_code'];
            $dpnom = $departement['departement_nom'];
...
<div class="modal-content">
    ...
    <p>Je recherche des </p>
        <label>Homme
        <input type="checkbox" name="homme" value="1" id="checkMen" <?php if($homme==1){echo "checked";} ?>/><br>
        </label> <br>

        <label>Femme
        <input type="checkbox" name="femme" value="1" id="checkWomen" <?php if($femme==1){echo "checked";} ?> /> <br>
        </label>
    ...
    <div>
    <option value="<?php if($departement_code!=-1){ echo $dpcode;} ?>">
    <?php
    if($departement_code!=-1){
        echo $dpnom;
    } 
    ...
    <label>Age minimum</label> <br>
        <input class="age_search" type="number" name="age_min" id="age_min" value="<?php if(!empty($ageMin)){echo $ageMin;} ?>" /><br>

     <label>Age maximum</label> <br>
        <input class="age_search" type="number" name="age_max" id="age_max" value="<?php if(!empty($ageMax)){echo $ageMax;} ?>" />
            ...
            <a href="profil.php?id=<?= $articles['id'] ?>">
                ...
                <div class="container_profil_info">
                    ...
                    <div class="container_profil_info_pseudo">
                        <h2><?php echo $articles['pseudo'] . $articles['sexe'] ?>
