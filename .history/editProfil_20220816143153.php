...
if(isset($_SESSION['id'])) //autorisation affichage page si compte existe
{
   ...
   //Modification  mail
    ...
    if($executeRequest==true){
        $updateProfil = $bdd->prepare('UPDATE membres SET pseudo=:pseudo, description_profil=:description_profil, astrologie=:astrologie, age=:age, mail=:mail, mdp =:mdp, avatar=:avatar, WHERE id = :id');
        $updateProfil->execute(array(
            ...
            'mdp' => $mdp,
            'avatar' => $avatar,
            'id' => Integer.parseInt($_SESSION['id']),
           ));
