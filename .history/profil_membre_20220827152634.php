...
if(isset($_GET['q']) AND !empty($_GET['q'])){
    ...
    $q_array = explode(' ', $q);
    $search = $bdd->query('SELECT CONCAT(pseudo,avatar,description_profil) FROM membres WHERE pseudo LIKE "%'.$q.'%" ORDER BY id DESC');
    if($search->rowCount() == 0 ){
        // SI AUCUN RESULTAT ON RECHERCHE PLUS LARGE
        var_dump('boucle entrer');
        $search = $bdd->query('SELECT pseudo FROM membres WHERE CONCAT(pseudo,avatar,description_profil) 
