...
<?php
  ...
  $commentaires = $bdd->prepare('SELECT * FROM commentaires WHERE id = ? ORDER BY id DESC');
  $commentaires->execute(array($commentaire));  
...
<?php while($c = $commentaires->fetch()) { ?>
   ...
   <?php 
   $reqGetMembreSender = $c['idMembreSender'];
   // RECUPERE LES COMMENTAIRE 
   $reqGetMembreSender = $bdd->prepare('SELECT * FROM membre WHERE id = ?');
   $reqGetMembreSender->execute(array($membreSender)); 
   $membreSender = $reqGetMembreSender->fetch(PDO::FETCH_ASSOC); 
?>
   <!-- -->

<b> <!-- <?= $membreSender['pseudo'] ?> --> PSEUDO :</b> <?= $c['commentaire'] ?><br />
