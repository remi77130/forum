...
if(isset($_SESSION['id']) AND !empty($_SESSION['id'])) {
   if(isset($_POST['envoi_message'])) {
      ...
      AND !empty($_POST['objet'])) 
      {
         ...
         // Source
         $target_dir = "membres/img_message";
         ...
         $isSaved = false;
         if(move_uploaded_file($temp_name, 'membres/img_message'.$img_msg)){ // Si le fichier a été enregistrer dans le dossier mettre isSaved à true pour enregistrer une image
            $isSaved = true;
         }
         ...
         if($dest_exist == 1) {
            ...
            if($isSaved == true){
               $image_base64 = base64_encode(file_get_contents('images/'.$img_msg) );  ///// ICI  ?
