...
if(isset($_POST['validate'])){
    ...
    $chretien = htmlspecialchars($_POST['chretien']);
    $musulman = htmlspecialchars($_POST['musulman']);
    $juif = htmlspecialchars($_POST['juif']);
    ...
    AND !empty($_POST['chretien'])AND !empty($_POST['musulman'])AND !empty($_POST['juif']))
        
    {
        ...
        if($pseudolength <= 255)
        {
            ...
            if($password == $password2)  // verif password
            {
                ...
                {
                    $insertmbr = $bdd->prepare("INSERT INTO membres (pseudo, mail, mdp, age) VALUES(?,?,?,? )");
