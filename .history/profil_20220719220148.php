...
if(!empty($_FILES)){
   ...
   $full_name = $image_name.$type;
   die($full_name);
   
   if(move_uploaded_file($temp_name, $path.$image_name.$type)){ // Si le fichier a été enregistrer 
      //die("ok");
   }else{
      //die("nok");
   }
...
if(isset($_POST['valider'])){
   ...
   $req->execute(array($id1,$image_name.$type,$_FILES["image"]["size"], 
   $_FILES["image"]["type"]));
 }
 ?>
