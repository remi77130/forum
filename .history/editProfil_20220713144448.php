...
////////////////////////////////////
   ...
   if(isset($_FILES['album']) AND !empty($_FILES['album']['name'])) {
      $tailleMax = 2097152; //2M0
      $extensionsValides = array('jpg', 'jpeg', 'gif', 'png');
      if($_FILES['album']['size'] <= $tailleMax) {
         $extensionUpload = strtolower(substr(strrchr($_FILES['album']['name'], '.'), 1));
         if(in_array($extensionUpload, $extensionsValides)) {
            $chemin = "membres\album/".$_SESSION['id'].".".$extensionUpload;
            $resultat = move_uploaded_file($_FILES['album']['tmp_name'], $chemin);
            if($resultat) {
               $updatealbum = $bdd->prepare('INSERT INTO membres album WHERE id = :id');
               $updatealbum->execute(array(
                  'album' => $_SESSION['id'].".".$extensionUpload,
                  'id' => $_SESSION['id']
                  ));
               header('Location: profil.php?id='.$_SESSION['id']);
            } else {
