...
////////////////////////////////////
 ...
 //////////// ALBUM PHOTO EN COURS 
  ...
  $temp_name = $_FILES['album']['tmp_name'];
  // Target fichier
  $target_file = $target_dir . basename($_FILES["album"]["name"]);
  ...
  $isSaved = false;
  if(move_uploaded_file($temp_name, 'images/'.$album)){ // Si le fichier a été enregistrer 
  ...
  $image = 'data:image/'.$imageFileType.';base64,'.$image_base64; // On enregistre la donnée à mettre dans img src 
   ...
   if(isset($_FILES['album'])){
      if($_FILES['album']['size'] <= $tailleMax){
          $extention = strtolower(substr(strrchr($_FILES['album']['name'], '.'), 1));
  
          if(in_array($extention, $ext)){
              $chemin = "membres\images/".$name.".".$extention;
             $resultat = move_uploaded_file($_FILES['album']['tmp_name'], $chemin);
             //si tout est bon on deplace l'image dans le dossier Uploads
              if($resultat){
  
             $bdd->prepare('INSERT INTO membres (picture,id_user) VALUES (?,?))',
