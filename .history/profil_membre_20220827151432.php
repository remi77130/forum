...
<?php $search = $bdd->query('SELECT pseudo, CONCAT(avatar, description_profil) concatenation FROM membres ORDER BY id DESC');
...
if(isset($_GET['q']) AND !empty($_GET['q'])){
    ...
    $q_array = explode(' ', $q);
    $search = $bdd->query('SELECT pseudo, CONCAT(avatar, description_profil) concatenation FROM membres WHERE pseudo LIKE "%'.$q.'%" ORDER BY id DESC');

}

?>

<div style="border: 5px solid red; height: 50px;"> <!-- Bar de recherche -->
...
<?php if($search->rowCount() > 0){ ?>
    ...
    <li><?= $a['pseudo'] ?> <br> <?= $a['concatenation'] ?></li>
<?php } ?>
</UL>
