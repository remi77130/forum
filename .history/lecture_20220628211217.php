...
if(isset($_SESSION['id']) AND !empty($_SESSION['id'])) {
   if(isset($_GET['id']) AND !empty($_GET['id'])) {
      ...
      $p_exp = $bdd->prepare('SELECT pseudo avatars FROM membres WHERE id = ?');
      $p_exp->execute(array($m['id_expediteur'] ['avatars']));
      $p_exp = $p_exp->fetch();
      $p_exp = $p_exp['pseudo'];
      $i_exp = $i_exp['avatars'];
?>
...
<body>
   ...
   <h3 align="center">Lecture du message de <?= $p_exp ?></h3> <?= $i_exp ?>
   <div align="center">
