...
{
   //INNER JOIN membres on membres.id=images.id WHERE membres.id=?");
    $requser = $bdd->prepare("SELECT * FROM membres INNER JOIN edit_profil on edit_profil.id=edit_profil. WHERE id = ?");
...
if(isset($_FILES['choix'])) {
   $newchoix = htmlspecialchars($_POST['choix']);
   $insertchoix = $bdd->prepare("UPDATE edit_profil SET choix = ? WHERE id = ?");
   $insertchoix->execute(array($newchoix, $_SESSION['id']));
