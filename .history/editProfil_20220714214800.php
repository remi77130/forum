...
////////////////////////////////////
   ...
   if(isset($_FILES['img']) AND !empty($_FILES['img']['name'])){ // SI UN FICHIER A BIEN ETE ENVOYER

      $tailleMax = 2097152; //2M0
      $extensionsValides = array('jpg', 'jpeg', 'gif', 'png');
      
      if($_FILES['img']['size'] <= $tailleMax)
      strtolower(substr(strrchr($_FILES['img']['name'], '.'), 1));
      ...
      if($resultat) {
      $updateimg = $bdd->prepare('INSERT INTO membres(img) VALUES (?)');
      //messages(id_expediteur,id_destinataire,message,objet,file_name,datafile) VALUES (?,?,?,?,?,?)');
      $updateavatar->execute(array(
